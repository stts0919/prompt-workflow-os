...
- `scripts/validate.py` `check_links` now strips `#fragment` before resolving a Markdown link target, so anchor links (`[label](path#section)`) no longer get falsely flagged as missing.
